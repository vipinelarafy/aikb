...
async function refresh() {
  setLoading(true);
  try {
    const r = await fetch(`/api/kb?ts=${Date.now()}`, { cache: 'no-store' });
...
// Auto-refresh: faster when KB is indexing, slower when complete/idle
useEffect(() => {
const intervalMs = kbStatus === 'in_progress' ? 3000 : 10000;
const id = setInterval(() => { refresh(); }, intervalMs);
return () => clearInterval(id);
}, [kbStatus]);
// refresh immediately when tab becomes visible (avoids stale status)
useEffect(() => {
const onVis = () => {
